...
function adaptive_timbre(hObject, handles, condition)

% randomize presentation order AND open subject data file
rand('seed', sum(100*clock()));
movegui(run_timbre_exp,'north');

% STIMULUS PARAMETER SPECIFICATION AND STIMULUS CREATION
Fs = 44100;
factor = [0.4 0.1 0.3; 0.2 0.05 0.15];  % adaptive step size in dB (20%, 10%)
curfactor = [0 0 0];
...
staircase = 1;
step = [1 1 1];
sax1 = wavread('C:\Users\Jeff\Documents\MATLAB\Neuro-Compensator\timbre perception\271\271TSNOM-22-F5');  % 1 second
...
while (reversal(condition) < 12 ) % || reversal(2) < 12 || reversal(3) < 12)
...
function [alteredsax1] = alter_sax(harm)
    ...
    elseif (harm == 2)
        ...
        sax1fft(44100-(1053+100)+1 : 44100-(1053-100)+1) = sax1fft(44100-(1053+100)+1 : 44100-(1053-100)+1) * curfactor(staircase); 
    end
...
function plot_sax_spectrum(altered)
    if (altered == 2)
        
        altsaxfft = fft(sax_stim(2*Fs+1:3*Fs));
        altsaxabs = abs(altsaxfft);
        figure(1); plot(1:6000,altsaxabs(1:6000));
        ylabel('Intensity');
        xlabel('Frequency (Hz)');
        title('Altered Sax Spectrum');
        
        unaltsaxfft = fft(sax_stim(1:Fs));
        unaltsaxabs = abs(unaltsaxfft);
        figure(2); plot(1:6000,unaltsaxabs(1:6000));
        ylabel('Intensity');
        xlabel('Frequency (Hz)');
        title('Unaltered Sax Spectrum');
        
    else
        
        altsaxfft = fft(sax_stim(4*Fs+1:5*Fs));
        altsaxabs = abs(altsaxfft);
        figure(1); plot(1:6000,altsaxabs(1:6000));
        ylabel('Intensity');
        xlabel('Frequency (Hz)');
