% vcv_heatmap_folder.m
% vcv_heatmap_folder(Cmatrices)
%
% Purpose: generate vcv heatmap between hearing aid conditions.
%
% Input: cell array (n subjects * 3 noise conditions), each cell containing
%        a 21x21 confusion matrix (type int)
%
...
function vcv_heatmap_folder(Cmatrices)

n = length(Cmatrices)/2;  % number of subjects
words = 12;  % the total number of words of each type that each participant
             % hears
...
title('VCV Confusion Matrix (Quiet) - NC','FontSize',16);
...
xlabel('Response','FontSize',14);
ylabel('Stimulus','FontSize',14);
title('VCV Confusion Matrix (Quiet) - WDRC','FontSize',16);
hold off
...
xlabel('Response','FontSize',14);
ylabel('Stimulus','FontSize',14);
title('VCV Confusion Matrix (Noise) - NC','FontSize',16);
...
xlabel('Response','FontSize',14);
ylabel('Stimulus','FontSize',14);
title('VCV Confusion Matrix (Noise) - WDRC','FontSize',16);
hold off
...
xlabel('Response','FontSize',14);
ylabel('Stimulus','FontSize',14);
title('VCV Confusion Matrix (Noise + Quiet) - NC','FontSize',16);
...
xlabel('Response','FontSize',14);
ylabel('Stimulus','FontSize',14);
title('VCV Confusion Matrix (Noise + Quiet) - WDRC','FontSize',16);
hold off
