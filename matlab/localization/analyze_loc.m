...
%
% input: subjname (arg1) (a string representing the subject name), num
% (arg2) (also a string, representing the subject number)
%
...
function [lowfreq,highfreq,phone,avgerror,ovrerror] = analyze_loc(subjname, num, varargin)
...
for j = 1:6
    ...
    M = M(:, [1:3 6:7]);  % use only relevant columns
    
    % take this out for second phase of early loc experiment
    %relabel();
    ...
    % CALCULATE RMS ERROR COLUMN FOR M
    %avgrms = sqrt(mean(M(:,5)));  % calculate average rms value
